...
Lemma congr_ifthenelse {s0 : tm} {s1 : tm} {s2 : tm} {t0 : tm} {t1 : tm}
  {t2 : tm} (H0 : s0 = t0) (H1 : s1 = t1) (H2 : s2 = t2) :
  ifthenelse s0 s1 s2 = ifthenelse t0 t1 t2.
Proof.
exact (eq_trans
         (eq_trans (eq_trans eq_refl (ap (fun x => ifthenelse x s1 s2) H0))
            (ap (fun x => ifthenelse t0 x s2) H1))
         (ap (fun x => ifthenelse t0 t1 x) H2)).
Qed.
...
Lemma congr_num {s0 : nat} {t0 : nat} (H0 : s0 = t0) : num s0 = num t0.
Proof.
exact (eq_trans eq_refl (ap (fun x => num x) H0)).
Qed.
...
Fixpoint ren_tm (xi_tm : nat -> nat) (s : tm) {struct s} : tm :=
  ...
  | false => false
  | ifthenelse s0 s1 s2 =>
      ifthenelse (ren_tm xi_tm s0) (ren_tm xi_tm s1) (ren_tm xi_tm s2)
...
Fixpoint subst_tm (sigma_tm : nat -> tm) (s : tm) {struct s} : tm :=
  ...
  | false => false
  | ifthenelse s0 s1 s2 =>
      ifthenelse (subst_tm sigma_tm s0) (subst_tm sigma_tm s1)
...
subst_tm sigma_tm s = s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (idSubst_tm sigma_tm Eq_tm s0)
...
ren_tm xi_tm s = ren_tm zeta_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (extRen_tm xi_tm zeta_tm Eq_tm s0)
...
subst_tm sigma_tm s = subst_tm tau_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (ext_tm sigma_tm tau_tm Eq_tm s0)
...
(s : tm) {struct s} : ren_tm zeta_tm (ren_tm xi_tm s) = ren_tm rho_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (compRenRen_tm xi_tm zeta_tm rho_tm Eq_tm s0)
...
subst_tm tau_tm (ren_tm xi_tm s) = subst_tm theta_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (compRenSubst_tm xi_tm tau_tm theta_tm Eq_tm s0)
...
ren_tm zeta_tm (subst_tm sigma_tm s) = subst_tm theta_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (compSubstRen_tm sigma_tm zeta_tm theta_tm Eq_tm s0)
...
subst_tm tau_tm (subst_tm sigma_tm s) = subst_tm theta_tm s :=
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (compSubstSubst_tm sigma_tm tau_tm theta_tm Eq_tm s0)
...
(Eq_tm : forall x, funcomp (var_tm) xi_tm x = sigma_tm x) (s : tm) {struct s}
  ...
  | false => congr_false
  | ifthenelse s0 s1 s2 =>
      congr_ifthenelse (rinst_inst_tm xi_tm sigma_tm Eq_tm s0)
