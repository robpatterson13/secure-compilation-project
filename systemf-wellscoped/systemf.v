...
Inductive ty (n_ty : nat) : Type :=
  ...
  | ex : ty (S n_ty) -> ty n_ty.

Lemma congr_bool {m_ty : nat} : bool m_ty = bool m_ty.
Proof.
exact (eq_refl).
Qed.
...
Lemma congr_ex {m_ty : nat} {s0 : ty (S m_ty)} {t0 : ty (S m_ty)}
  (H0 : s0 = t0) : ex m_ty s0 = ex m_ty t0.
Proof.
exact (eq_trans eq_refl (ap (fun x => ex m_ty x) H0)).
Qed.
...
(s : ty m_ty) {struct s} : ty n_ty :=
  ...
  | ex _ s0 => ex n_ty (ren_ty (upRen_ty_ty xi_ty) s0)
  end.
...
(s : ty m_ty) {struct s} : ty n_ty :=
  ...
  | ex _ s0 => ex n_ty (subst_ty (up_ty_ty sigma_ty) s0)
  end.
...
Inductive tm (n_ty n_vl : nat) : Type :=
  ...
  | unpack : tm n_ty n_vl -> tm (S n_ty) (S n_vl) -> tm n_ty n_vl
with vl (n_ty n_vl : nat) : Type :=
  ...
  | pack : ty n_ty -> vl n_ty n_vl -> vl n_ty n_vl.
...
Lemma congr_unpack {m_ty m_vl : nat} {s0 : tm m_ty m_vl}
  {s1 : tm (S m_ty) (S m_vl)} {t0 : tm m_ty m_vl} {t1 : tm (S m_ty) (S m_vl)}
  (H0 : s0 = t0) (H1 : s1 = t1) :
  unpack m_ty m_vl s0 s1 = unpack m_ty m_vl t0 t1.
Proof.
exact (eq_trans (eq_trans eq_refl (ap (fun x => unpack m_ty m_vl x s1) H0))
         (ap (fun x => unpack m_ty m_vl t0 x) H1)).
Qed.

Lemma congr_true {m_ty m_vl : nat} : true m_ty m_vl = true m_ty m_vl.
Proof.
exact (eq_refl).
Qed.

Lemma congr_false {m_ty m_vl : nat} : false m_ty m_vl = false m_ty m_vl.
Proof.
exact (eq_refl).
Qed.
...
Lemma congr_pack {m_ty m_vl : nat} {s0 : ty m_ty} {s1 : vl m_ty m_vl}
  {t0 : ty m_ty} {t1 : vl m_ty m_vl} (H0 : s0 = t0) (H1 : s1 = t1) :
  pack m_ty m_vl s0 s1 = pack m_ty m_vl t0 t1.
Proof.
exact (eq_trans (eq_trans eq_refl (ap (fun x => pack m_ty m_vl x s1) H0))
         (ap (fun x => pack m_ty m_vl t0 x) H1)).
Qed.
...
(s : tm m_ty m_vl) {struct s} : tm n_ty n_vl :=
  ...
  | unpack _ _ s0 s1 =>
      unpack n_ty n_vl (ren_tm xi_ty xi_vl s0)
...
(s : vl m_ty m_vl) {struct s} : vl n_ty n_vl :=
  ...
  | true _ _ => true n_ty n_vl
  | false _ _ => false n_ty n_vl
  | lam _ _ s0 s1 =>
  ...
  | pack _ _ s0 s1 =>
      pack n_ty n_vl (ren_ty xi_ty s0) (ren_vl xi_ty xi_vl s1)
  end.
...
(s : tm m_ty m_vl) {struct s} : tm n_ty n_vl :=
  ...
  | unpack _ _ s0 s1 =>
      unpack n_ty n_vl (subst_tm sigma_ty sigma_vl s0)
...
(s : vl m_ty m_vl) {struct s} : vl n_ty n_vl :=
  ...
  | pack _ _ s0 s1 =>
      pack n_ty n_vl (subst_ty sigma_ty s0) (subst_vl sigma_ty sigma_vl s1)
  end.
...
Lemma renRen'_tm_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (xi_ty : fin m_ty -> fin k_ty)
  (xi_vl : fin m_vl -> fin k_vl) (zeta_ty : fin k_ty -> fin l_ty)
  (zeta_vl : fin k_vl -> fin l_vl) :
...
Lemma renRen'_vl_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (xi_ty : fin m_ty -> fin k_ty)
  (xi_vl : fin m_vl -> fin k_vl) (zeta_ty : fin k_ty -> fin l_ty)
  (zeta_vl : fin k_vl -> fin l_vl) :
  pointwise_relation _ eq
...
Lemma renSubst_tm_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (xi_ty : fin m_ty -> fin k_ty)
  (xi_vl : fin m_vl -> fin k_vl) (tau_ty : fin k_ty -> ty l_ty)
  (tau_vl : fin k_vl -> vl l_ty l_vl) :
  pointwise_relation _ eq
...
Lemma renSubst_vl_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (xi_ty : fin m_ty -> fin k_ty)
  (xi_vl : fin m_vl -> fin k_vl) (tau_ty : fin k_ty -> ty l_ty)
  (tau_vl : fin k_vl -> vl l_ty l_vl) :
  pointwise_relation _ eq
...
Lemma substRen_tm_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (sigma_ty : fin m_ty -> ty k_ty)
  (sigma_vl : fin m_vl -> vl k_ty k_vl) (zeta_ty : fin k_ty -> fin l_ty)
  (zeta_vl : fin k_vl -> fin l_vl) :
  pointwise_relation _ eq
...
Lemma substRen_vl_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (sigma_ty : fin m_ty -> ty k_ty)
  (sigma_vl : fin m_vl -> vl k_ty k_vl) (zeta_ty : fin k_ty -> fin l_ty)
  (zeta_vl : fin k_vl -> fin l_vl) :
  pointwise_relation _ eq
...
Lemma substSubst_tm_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (sigma_ty : fin m_ty -> ty k_ty)
  (sigma_vl : fin m_vl -> vl k_ty k_vl) (tau_ty : fin k_ty -> ty l_ty)
  (tau_vl : fin k_vl -> vl l_ty l_vl) :
  pointwise_relation _ eq
...
Lemma substSubst_vl_pointwise {k_ty k_vl : nat} {l_ty l_vl : nat}
  {m_ty m_vl : nat} (sigma_ty : fin m_ty -> ty k_ty)
  (sigma_vl : fin m_vl -> vl k_ty k_vl) (tau_ty : fin k_ty -> ty l_ty)
  (tau_vl : fin k_vl -> vl l_ty l_vl) :
  pointwise_relation _ eq
...
Lemma rinstInst'_tm_pointwise {m_ty m_vl : nat} {n_ty n_vl : nat}
  (xi_ty : fin m_ty -> fin n_ty) (xi_vl : fin m_vl -> fin n_vl) :
...
Lemma rinstInst'_vl_pointwise {m_ty m_vl : nat} {n_ty n_vl : nat}
  (xi_ty : fin m_ty -> fin n_ty) (xi_vl : fin m_vl -> fin n_vl) :
...
Lemma instId'_tm_pointwise {m_ty m_vl : nat} :
  pointwise_relation _ eq (subst_tm (var_ty m_ty) (var_vl m_ty m_vl)) id.
Proof.
exact (fun s =>
...
Lemma instId'_vl_pointwise {m_ty m_vl : nat} :
  pointwise_relation _ eq (subst_vl (var_ty m_ty) (var_vl m_ty m_vl)) id.
Proof.
exact (fun s =>
...
Lemma rinstId'_tm_pointwise {m_ty m_vl : nat} :
  pointwise_relation _ eq (@ren_tm m_ty m_vl m_ty m_vl id id) id.
Proof.
exact (fun s =>
...
Lemma rinstId'_vl_pointwise {m_ty m_vl : nat} :
  pointwise_relation _ eq (@ren_vl m_ty m_vl m_ty m_vl id id) id.
Proof.
exact (fun s =>
...
Lemma varL'_vl_pointwise {m_ty m_vl : nat} {n_ty n_vl : nat}
  (sigma_ty : fin m_ty -> ty n_ty) (sigma_vl : fin m_vl -> vl n_ty n_vl) :
  pointwise_relation _ eq
...
Lemma varLRen'_vl_pointwise {m_ty m_vl : nat} {n_ty n_vl : nat}
  (xi_ty : fin m_ty -> fin n_ty) (xi_vl : fin m_vl -> fin n_vl) :
  pointwise_relation _ eq (funcomp (ren_vl xi_ty xi_vl) (var_vl m_ty m_vl))
    (funcomp (var_vl n_ty n_vl) xi_vl).
Proof.
exact (fun x => eq_refl).
Qed.
...
#[global]
Instance subst_vl_morphism2  {m_ty m_vl : nat} {n_ty n_vl : nat}:
 (Proper
    (respectful (pointwise_relation _ eq)
...
#[global]
Instance subst_tm_morphism2  {m_ty m_vl : nat} {n_ty n_vl : nat}:
 (Proper
    (respectful (pointwise_relation _ eq)
       (respectful (pointwise_relation _ eq) (pointwise_relation _ eq)))
    (@subst_tm m_ty m_vl n_ty n_vl)).
...
#[global]
Instance ren_vl_morphism2  {m_ty m_vl : nat} {n_ty n_vl : nat}:
 (Proper
    (respectful (pointwise_relation _ eq)
       (respectful (pointwise_relation _ eq) (pointwise_relation _ eq)))
    (@ren_vl m_ty m_vl n_ty n_vl)).
...
#[global]
Instance ren_tm_morphism2  {m_ty m_vl : nat} {n_ty n_vl : nat}:
 (Proper
    (respectful (pointwise_relation _ eq)
       (respectful (pointwise_relation _ eq) (pointwise_relation _ eq)))
    (@ren_tm m_ty m_vl n_ty n_vl)).
...
Arguments pack {n_ty n_vl}.

Arguments tlam {n_ty n_vl}.

Arguments lam {n_ty n_vl}.

Arguments false {n_ty n_vl}.

Arguments true {n_ty n_vl}.

Arguments unpack {n_ty n_vl}.
